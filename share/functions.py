...
def MB(mass, P_ref):
    ...
    T       = 1.0                      # K
    Da      = (1.66053907)*(10**-27) # kg

    velocities   = [0.0, 500.0]
    steps        = 1
    
    mass = mass*Da

    for x in np.arange(velocities[0], velocities[1], steps):
        term1 = np.sqrt(2/np.pi)
        term2 = x**2
        term3 = np.exp(-(x**2)/(2*(np.sqrt((kB*T)/mass))**2))
        term4 = (np.sqrt((kB*T)/mass))**3
